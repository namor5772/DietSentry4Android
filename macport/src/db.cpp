...
// interchangeable between the apps. macOS deltas from winport/src/db.cpp:
// POSIX file copy/delete, strcasecmp, '/' separators, bundle-aware assets dir.
#include "app.h"
#include "sqlite3.h"
#include <algorithm>
#include <strings.h>
...
static std::wstring assetDbPath() {
    return assetsDirectory() + L"/foods.db";
}
...
static bool tableHasColumn(sqlite3* db, const char* table, const char* column) {
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (name && strcasecmp(name, column) == 0) { found = true; break; }
        }
    }
...
static int columnIndex(sqlite3_stmt* s, const char* name) {
    ...
    for (int i = 0; i < n; i++) {
        if (strcasecmp(sqlite3_column_name(s, i), name) == 0) return i;
    }
    return -1;
}
...
bool Db::replaceDatabaseFrom(const std::wstring& srcPath) {
    ...
    if (!copyFile(srcPath, tmp)) return false;
    close();
    bool ok = copyFile(tmp, dst);
    deleteFile(tmp);
