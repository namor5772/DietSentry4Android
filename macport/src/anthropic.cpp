// anthropic.cpp — libcurl port of the Kotlin callAnthropicApi + helpers.
// Mirrors winport/src/anthropic.cpp exactly except the HTTP transport:
// request JSON shape, prompt-cache breakpoints, client tool loop
...
// ---------------------------------------------------------------------------
// libcurl transport (macOS ships libcurl; timeouts match the Android client:
// connect 30s, overall 240s)
// ---------------------------------------------------------------------------
struct HttpResult {
...
static size_t curlAppendBody(char* ptr, size_t size, size_t nmemb, void* userdata) {
...
static HttpResult httpPostAnthropicMessages(const std::string& apiKey, const std::string& body) {
    ...
    if (!curl) { out.transportError = "curl_easy_init failed"; return out; }

    struct curl_slist* headers = nullptr;
    headers = curl_slist_append(headers, ("x-api-key: " + apiKey).c_str());
    headers = curl_slist_append(headers, (std::string("anthropic-version: ") + ANTHROPIC_VERSION).c_str());
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "DietSentry4Mac/1.0");
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, curlAppendBody);
    curl_easy_setopt(curl, CURLOPT_WRITEDATA, &out.body);
    curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1L);   // worker-thread safety
    ...
    if (rc != CURLE_OK) {
        if (rc == CURLE_OPERATION_TIMEDOUT)
            ...
            out.transportError = "No network connection to api.anthropic.com";
        else
            out.transportError = std::string("Network error (") + curl_easy_strerror(rc) + ")";
