...
// (with a remembered exchange folder — the macOS equivalent of the Android
// SAF folder flow, using NSOpenPanel via imageutil), Eaten Graph entry,
// Weight Table CRUD. Mirrors winport/src/screens_utilities.cpp; deltas:
// pickFolder lives in imageutil.mm, directoryExists replaces
// GetFileAttributesW, '/' path separators.
#include "app.h"
#include "ui.h"
#include "helptexts.h"
#include "imageutil.h"
...
struct UtilitiesScreen : Screen {
    ...
    bool ensureExchangeFolder(App& app) {
        if (!exchangeFolder.empty()) {
            if (directoryExists(exchangeFolder)) return true;
