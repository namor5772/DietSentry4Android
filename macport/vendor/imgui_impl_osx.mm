...
static bool ImGui_ImplOSX_HandleEvent(NSEvent* event, NSView* view)
{
    ...
    if (event.type == NSEventTypeKeyDown || event.type == NSEventTypeKeyUp)
    {
        ...
        // [DietSentry patch] Synthetic input (VNC/remote-desktop servers via
        // CGEventPost) often embeds modifier flags in the key event without
        // emitting a separate NSEventTypeFlagsChanged, leaving io.KeyCtrl etc.
        // stale. Sync modifiers from every key event as well (same defensive
        // pattern as the GLFW/SDL backends); harmless for physical keyboards.
