...
- Utilities:
  ...
  - `Export db as…` / `Import db from…` (**Android only**): single-file export/import through the system file picker, which reaches cloud storage (e.g. OneDrive, Google Drive) that the remembered exchange folder cannot — cloud providers don't support Android's folder picker. Import validates the SQLite header before replacing anything. The desktop ports don't need these buttons: their exchange folder is a plain filesystem path that can point directly at a locally synced cloud folder.
...
3. **Via OneDrive (or another cloud drive), no manual transfer**: on the PC point the exchange folder at a folder inside the locally synced OneDrive tree (e.g. `OneDrive\MyImportant\DS`). On Android use Utilities → `Export db as…` and save straight into that same OneDrive folder in the system file picker; the reverse trip uses `Import db from…` and picks the file from OneDrive. (The Android exchange folder itself can't be a cloud folder — cloud providers don't appear in Android's folder picker — hence the single-file buttons.)
