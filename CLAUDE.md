...
**Desktop keyboard navigation (both C++ ports, 2026-08-17).** ImGui keyboard nav is on (`ImGuiConfigFlags_NavEnableKeyboard`) and the shared UI layer makes it work end to end: Tab/Shift+Tab in visual order (`ui::topBar` submits its actions left→right on purpose), a visible purple focus ring (`ImGuiCol_NavCursor`), Enter/Space activate, Escape closes the top-most modal (`beginDialog` and the pickers handle it — ImGui itself only auto-closes non-modal popups) or goes back a screen. Conventions to keep it working: any custom hit-target is submitted with `ui::navHitTarget()` (an `InvisibleButton` with `ImGuiButtonFlags_EnableNav` — InvisibleButton is *excluded* from nav by default — and ImGui's own expanded ring suppressed) and draws `ui::focusRing()` after painting its background, so exactly one compact ring shows; `ui::virtualList` owns each row's hit-target (`draw(i, w, pressed)` — never test `IsItemClicked()` yourself, it is mouse-only), is nav-flattened, exposes exactly one Tab stop and overscans a few rows so arrows scroll; scroll children that contain controls need `ImGuiChildFlags_NavFlattened`, read-only text regions use `ui::beginTextScroll/endTextScroll`; destructive dialogs call `ui::dialogNoDefaultFocus()` after `beginDialog` so a second Enter cannot delete; `App::endFrameKeyboardNav()` (called by both mains after the screen draws) drops a focus id that no longer exists so Tab restarts from the first control after a screen change — but only once the id has been dead at the end of two consecutive frames, because a closing popup hands focus back to its opener (combo, date button) after that control was already drawn; both mains set `io.ConfigNavEscapeClearFocusItem = false` because ImGui's default reaction to an Escape nothing consumes (our Back key at the root screen, or on any screen with a focused control) is to clear the focused item while leaving the cursor flag on, after which Tab has no item to step from and is dead until an arrow key or the mouse is used. **Quitting from the keyboard**: Escape never quits — on the root Foods Table `onBack` deliberately returns true, so a stray extra press stays harmless — instead `App::pollGlobalShortcuts()` (shared `ui.cpp`, called by both mains right after `endFrameKeyboardNav()`) polls the exact chord `ImGuiMod_Ctrl | ImGuiKey_Q` with a plain `IsKeyChordPressed` (no routing, so it fires whichever window has focus, inside modals and while a text field is mid-edit) and sets `App::quitRequested`, which each main already turns into a clean exit (Windows: leaves the frame loop → `db.close()` → shutdown; macOS: `[NSApp terminate:]`). ImGui maps the Cmd key to `ImGuiMod_Ctrl` on macOS, so the one chord is Ctrl+Q on Windows and Cmd+Q on the Mac (where the app menu's *Quit DietSentry* item normally handles Cmd+Q first); Alt+F4 / the close button keep working (verified 2026-08-17 on Windows: Ctrl+Q at the root, on a sub-screen, with the help sheet open and mid-edit; Ctrl+Shift+Q does nothing). Test hooks: `DIETSENTRY_AUTONAV=<route>` and `DIETSENTRY_DATA_DIR=<folder>` (isolated foods.db/prefs); on Windows the app can be driven without focus by posting `WM_KEYDOWN/WM_KEYUP/WM_CHAR` to its window and captured with `PrintWindow`; the ImGui Win32 backend reads modifiers from the app thread's key state (`GetKeyState`), so a modifier chord (Shift+Tab, Ctrl+Q) is posted by `AttachThreadInput` to the app's thread, `SetKeyboardState` with the modifier bit set, posting the key, waiting a few frames, then clearing the state (`SendInput`/`keybd_event` would need the window in the foreground; Alt+F4 is `WM_SYSKEYDOWN VK_F4` with lParam bit 29). Gotcha when scripting text fields: Tab *into* an `InputText` already starts editing (ImGui activates it `FromTabbing`), so a posted Enter right after Tab *finishes* that edit rather than starting one — type straight away, or Enter twice; on macOS (verified 2026-08-17 with the app frontmost) real HID key events posted with `CGEventPost` go through AppKit → `imgui_impl_osx.mm` unchanged — Tab/Shift+Tab/Escape/Return are not swallowed (`interpretKeyEvents:` only turns them into no-op `insertTab:`/`insertBacktab:`/`cancelOperation:` selectors, plus an `insertText:` of `\r` for Return that ImGui filters out) — and `screencapture -l <windowid>` captures the window (both need the Terminal's Accessibility / Screen Recording permission).
