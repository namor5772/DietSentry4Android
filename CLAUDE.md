...
**Desktop keyboard navigation (both C++ ports, 2026-08-17).** ImGui keyboard nav is on (`ImGuiConfigFlags_NavEnableKeyboard`) and the shared UI layer makes it work end to end: Tab/Shift+Tab in visual order (`ui::topBar` submits its actions left→right on purpose), a visible purple focus ring (`ImGuiCol_NavCursor`), Enter/Space activate, Escape closes the top-most modal (`beginDialog` and the pickers handle it — ImGui itself only auto-closes non-modal popups) or goes back a screen. Conventions to keep it working: any custom `ImGui::InvisibleButton` hit-target must pass `ImGuiButtonFlags_EnableNav` (InvisibleButton is *excluded* from nav by default) and draw `ui::focusRing()` after painting its background; `ui::virtualList` owns each row's hit-target (`draw(i, w, pressed)` — never test `IsItemClicked()` yourself, it is mouse-only), is nav-flattened, exposes exactly one Tab stop and overscans a few rows so arrows scroll; scroll children that contain controls need `ImGuiChildFlags_NavFlattened`, read-only text regions use `ui::beginTextScroll/endTextScroll`; destructive dialogs call `ui::dialogNoDefaultFocus()` after `beginDialog` so a second Enter cannot delete; `App::endFrameKeyboardNav()` (called by both mains after the screen draws) drops a focus id that no longer exists so Tab restarts from the first control after a screen change. Test hooks: `DIETSENTRY_AUTONAV=<route>` and `DIETSENTRY_DATA_DIR=<folder>` (isolated foods.db/prefs); on Windows the app can be driven without focus by posting `WM_KEYDOWN/WM_KEYUP/WM_CHAR` to its window and captured with `PrintWindow` (Shift+Tab cannot be simulated that way — modifiers come from the real keyboard state).
